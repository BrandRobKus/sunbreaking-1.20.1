        ...
        if (user instanceof PlayerEntity playerEntity) {
            ...
            if (i >= USE_THRESHOLD) {
                if (!world.isClient) {
                    ...
                    HellbentHammerProjectileEntity HellbentHammerEntity = new HellbentHammerProjectileEntity(world, playerEntity, stack);

                    float velocity = hasBulk(stack) ? PROJECTILE_SPEED / 1.33F : PROJECTILE_SPEED;

                    System.out.println("HellbentHammerProjectileEntity velocity: " + velocity);

                    HellbentHammerEntity.setVelocity(playerEntity, playerEntity.getPitch(), playerEntity.getYaw(), 0.0F, velocity, 1.0F);

                    if (playerEntity.getAbilities().creativeMode) {
                        HellbentHammerEntity.pickupType = PersistentProjectileEntity.PickupPermission.CREATIVE_ONLY;
                    }

                    world.spawnEntity(HellbentHammerEntity);

                    world.playSoundFromEntity(null, HellbentHammerEntity, ModSounds.HAMMER_THROW, SoundCategory.PLAYERS,
                            2.0F, 1.0F);
