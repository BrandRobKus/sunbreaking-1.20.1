...
public class BlazingHammerItem extends TridentItem {
    public static final int USE_THRESHOLD = 10;
    public static final float PROJECTILE_SPEED = 2.5F;
    private final Multimap<EntityAttribute, EntityAttributeModifier> attributeModifiers;
    public BlazingHammerItem(Settings settings) {
        super(settings.maxDamage(350));
        ImmutableMultimap.Builder<EntityAttribute, EntityAttributeModifier> builder = ImmutableMultimap.builder();
        builder.put(
                EntityAttributes.GENERIC_ATTACK_DAMAGE,
                new EntityAttributeModifier(ATTACK_DAMAGE_MODIFIER_ID, "Tool modifier", 9.0, EntityAttributeModifier.Operation.ADDITION)
        );
        builder.put(
        ...
        if (user instanceof PlayerEntity playerEntity) {
            ...
            if (i >= USE_THRESHOLD) {
                if (!world.isClient) {
                    ...
                    BlazingHammerProjectileEntity BlazingHammerEntity = new BlazingHammerProjectileEntity(world, playerEntity, stack);

                    float velocity = hasBulk(stack) ? PROJECTILE_SPEED / 1.33F : PROJECTILE_SPEED;

                    System.out.println("BlazingHammerProjectileEntity velocity: " + velocity);

                    BlazingHammerEntity.setVelocity(playerEntity, playerEntity.getPitch(), playerEntity.getYaw(), 0.0F, velocity, 1.0F);

                    if (playerEntity.getAbilities().creativeMode) {
                        BlazingHammerEntity.pickupType = PersistentProjectileEntity.PickupPermission.CREATIVE_ONLY;
                    }

                    world.spawnEntity(BlazingHammerEntity);

                    world.playSoundFromEntity(null, BlazingHammerEntity, ModSounds.HAMMER_THROW, SoundCategory.PLAYERS,
                            2.0F, 1.0F);
