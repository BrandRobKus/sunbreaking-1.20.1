        ...
        if (user instanceof PlayerEntity playerEntity) {
            ...
            if (i >= USE_THRESHOLD) {
                if (!world.isClient) {
                    ...
                    FlatHammerProjectileEntity FlatHammerEntity = new FlatHammerProjectileEntity(world, playerEntity, stack);

                    float velocity = hasBulk(stack) ? PROJECTILE_SPEED / 1.33F : PROJECTILE_SPEED;

                    System.out.println("FlatHammerProjectileEntity velocity: " + velocity);

                    FlatHammerEntity.setVelocity(playerEntity, playerEntity.getPitch(), playerEntity.getYaw(), 0.0F, velocity, 1.0F);

                    if (playerEntity.getAbilities().creativeMode) {
                        FlatHammerEntity.pickupType = PersistentProjectileEntity.PickupPermission.CREATIVE_ONLY;
                    }

                    world.spawnEntity(FlatHammerEntity);

                    world.playSoundFromEntity(null, FlatHammerEntity, ModSounds.HAMMER_THROW, SoundCategory.PLAYERS,
                            2.0F, 1.0F);
